...
//!     let builder = oximqtt::net::Builder::new()
//!         .name("MyMQTTBroker")
...
#[cfg(feature = "quic")]
use crate::net::quic::QuinnBiStream;
use crate::net::stream::Dispatcher;
#[cfg(feature = "ws")]
use crate::net::ws::WsStream;
#[cfg(feature = "tls")]
use crate::net::TlsCertExtractor;
...
/// # Examples
/// ```
/// # use oximqtt::net::{Builder, Listener};
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    pub async fn tls(self) -> Result<Dispatcher<TlsStream<S>>> {
        ...
        let acceptor = self.acceptor.ok_or_else(|| crate::net::MqttError::ServiceUnavailable)?;
        let tls_s = match tokio::time::timeout(self.cfg.handshake_timeout, acceptor.accept(self.socket)).await
        {
            Ok(Ok(tls_s)) => tls_s,
            Ok(Err(e)) => return Err(e.into()),
            Err(_) => return Err(crate::net::MqttError::ReadTimeout.into()),
        };
    ...
    pub async fn ws(self) -> Result<Dispatcher<WsStream<S>>> {
            ...
            .await
        {
            ...
            Ok(Err(e)) => Err(e.into()),
            Err(_) => Err(crate::net::MqttError::ReadTimeout.into()),
        }
    }
