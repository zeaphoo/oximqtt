...
use crate::net::error::MqttError;
use crate::net::{Builder, Result};
use crate::codec::cert::CertInfo;
use crate::codec::error::{DecodeError, SendPacketError};
use crate::codec::v3::Codec as CodecV3;
use crate::codec::v5::Codec as CodecV5;
use crate::codec::version::{ProtocolVersion, VersionCodec};
use crate::codec::{MqttCodec, MqttPacket};
...
pub mod v3 {
    ...
    use crate::net::error::MqttError;
    use crate::net::{Builder, Error, Result};
    #[cfg(feature = "tls")]
    use crate::codec::cert::CertInfo;
    use crate::codec::error::DecodeError;
    use crate::codec::types::Publish;
    use crate::codec::v3::{Connect, ConnectAckReason, Packet as PacketV3, Packet};
    use crate::codec::{MqttCodec, MqttPacket};
    ...
    /// use tokio_util::codec::Framed;
    /// use oximqtt::codec::{MqttCodec, types::Publish};
    /// use oximqtt::net::{Builder,v3};
    ///
        ...
        Io: AsyncRead + AsyncWrite + Unpin,
    {
        ...
        pub async fn send_subscribe_ack(
            ...
            status: Vec<crate::codec::v3::SubscribeReturnCode>,
        ) -> Result<()> {
        ...
        #[inline]
        pub async fn send_connect(&mut self, connect: crate::codec::v3::Connect) -> Result<()> {
        ...
        pub async fn send_connect_ack(
            ...
            self.send(PacketV3::ConnectAck(crate::codec::v3::ConnectAck { session_present, return_code }))
                .await
        }
        ...
        #[inline]
        pub async fn send(&mut self, packet: crate::codec::v3::Packet) -> Result<()> {
        ...
        #[inline]
        pub async fn recv(&mut self, tm: Duration) -> Result<Option<crate::codec::v3::Packet>> {
    ...
    where
        Io: AsyncRead + Unpin,
    {
        type Item = Result<crate::codec::v3::Packet>;
...
pub mod v5 {
    ...
    use crate::codec::error::DecodeError;
    use crate::codec::types::Publish;
    use crate::codec::v5::{Auth, Connect, Disconnect, Packet as PacketV5, Packet};
    use crate::codec::{MqttCodec, MqttPacket};
    ...
    /// use tokio_util::codec::Framed;
    /// use oximqtt::codec::{MqttCodec, types::Publish};
    /// use oximqtt::net::{Builder,v5};
    /// use oximqtt::codec::v5::Connect;
    ///
    /// # async fn example() -> Result<(), Box<dyn std::error::Error>> {
    /// let addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 1883);
    /// let stream = TcpStream::connect(addr).await?;
    ...
    /// mqtt_stream.send_auth(oximqtt::codec::v5::Auth::default()).await?;
    /// # Ok(())
    /// # }
    /// ```
    impl<Io> MqttStream<Io>
        ...
        Io: AsyncRead + AsyncWrite + Unpin,
    {
        ...
        /// Acknowledges a received publish (QoS 1)
        #[inline]
        pub async fn send_publish_ack(&mut self, ack: crate::codec::v5::PublishAck) -> Result<()> {
            self.send(PacketV5::PublishAck(ack)).await
        }

        /// Confirms receipt of a publish (QoS 2 step 1)
        #[inline]
        pub async fn send_publish_received(&mut self, ack: crate::codec::v5::PublishAck) -> Result<()> {
            self.send(PacketV5::PublishReceived(ack)).await
        }

        /// Releases a stored publish (QoS 2 step 2)
        #[inline]
        pub async fn send_publish_release(&mut self, ack2: crate::codec::v5::PublishAck2) -> Result<()> {
            self.send(PacketV5::PublishRelease(ack2)).await
        }

        /// Confirms publish completion (QoS 2 step 3)
        #[inline]
        pub async fn send_publish_complete(&mut self, ack2: crate::codec::v5::PublishAck2) -> Result<()> {
            self.send(PacketV5::PublishComplete(ack2)).await
        }

        /// Acknowledges a subscription request
        #[inline]
        pub async fn send_subscribe_ack(&mut self, ack: crate::codec::v5::SubscribeAck) -> Result<()> {
            self.send(PacketV5::SubscribeAck(ack)).await
        }

        /// Acknowledges an unsubscribe request
        #[inline]
        pub async fn send_unsubscribe_ack(&mut self, unack: crate::codec::v5::UnsubscribeAck) -> Result<()> {
            self.send(PacketV5::UnsubscribeAck(unack)).await
        }

        /// Initiates connection to the broker
        #[inline]
        pub async fn send_connect(&mut self, connect: crate::codec::v5::Connect) -> Result<()> {
            self.send(PacketV5::Connect(Box::new(connect))).await
        }

        /// Responds to connection request
        #[inline]
        pub async fn send_connect_ack(&mut self, ack: crate::codec::v5::ConnectAck) -> Result<()> {
        ...
        /// Generic packet sending method
        #[inline]
        pub async fn send(&mut self, packet: crate::codec::v5::Packet) -> Result<()> {
        ...
        /// Receives next packet with timeout
        #[inline]
        pub async fn recv(&mut self, tm: Duration) -> Result<Option<crate::codec::v5::Packet>> {
            match tokio::time::timeout(tm, self.next()).await {
                Ok(Some(Ok(msg))) => Ok(Some(msg)),
                Ok(Some(Err(e))) => Err(e),
                Ok(None) => Ok(None),
    ...
    where
        Io: AsyncRead + Unpin,
    {
        type Item = Result<crate::codec::v5::Packet>;
