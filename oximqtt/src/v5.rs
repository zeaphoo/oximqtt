...
async fn _handshake(
    ...
    // instead of refusing the client with ServerUnavailable immediately.
    let mut entry = {
        let mut attempts = 0;
        loop {
    ...
    let shared_subscription_available = state.scx.extends.shared_subscription().await.is_supported(state.listen_cfg());
