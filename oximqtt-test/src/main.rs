//! mqtt_harness - OXIMQTT Industrial-Grade Test Harness
//!
//! Entry point for the OXIMQTT broker test and benchmarking system.
...
#[derive(Debug, Parser)]
#[command(name = "mqtt_harness", about = "OXIMQTT Industrial-Grade Test Harness")]
struct Opt {
    /// Broker address (default: 127.0.0.1:1883)
    #[arg(short, long, default_value = "127.0.0.1:1883")]
    addr: String,

    /// Path to oximqttd binary
    #[arg(short, long)]
    binary: Option<String>,

    /// Path to oximqtt.toml config
    #[arg(short = 'c', long)]
    config: Option<String>,

    /// Workspace root (for finding target/release/oximqttd)
    #[arg(long)]
...
fn main() {
    ...
    if let Some(ref json_path) = opt.json {
        let report = JsonReporter::generate("oximqtt-test", results, &summary);
...
mod cli_tests {
    ...
    fn test_binary_short() {
        let opt = Opt::parse_from(["mqtt_harness", "-b", "./target/release/oximqttd"]);
        assert_eq!(opt.binary.as_deref(), Some("./target/release/oximqttd"));
    }

    #[test]
    fn test_binary_long() {
        let opt = Opt::parse_from(["mqtt_harness", "--binary", "/usr/bin/oximqttd"]);
        assert_eq!(opt.binary.as_deref(), Some("/usr/bin/oximqttd"));
    }

    #[test]
    fn test_config_short() {
        let opt = Opt::parse_from(["mqtt_harness", "-c", "config/oximqtt.toml"]);
        assert_eq!(opt.config.as_deref(), Some("config/oximqtt.toml"));
    }

    #[test]
    fn test_config_long() {
        let opt = Opt::parse_from(["mqtt_harness", "--config", "/etc/oximqtt/oximqtt.toml"]);
        assert_eq!(opt.config.as_deref(), Some("/etc/oximqtt/oximqtt.toml"));
    }

    #[test]
    fn test_workspace() {
        let opt = Opt::parse_from(["mqtt_harness", "--workspace", "/home/user/oximqtt"]);
        assert_eq!(opt.workspace.as_deref(), Some("/home/user/oximqtt"));
    }
    ...
    fn test_all_options() {
        ...
        assert_eq!(opt.addr, "0.0.0.0:1883");
        assert_eq!(opt.binary.as_deref(), Some("oximqttd"));
        assert_eq!(opt.config.as_deref(), Some("myconfig.toml"));
        assert_eq!(opt.workspace.as_deref(), Some("/opt/oximqtt"));
