...
Starting from *OXIMQTT 0.8.0*, support has been added for using an access control list to specify multiple permissions, 
...
```json
{
  ...
  "acl": [
    {
      // Allows the client to publish messages to the topic `foo/${clientid}`, for example, `foo/oximqtt_c`.
      "permission": "allow",
      "action": "publish",
      "topic": "foo/${clientid}"
    },
    ...
    {
     // Allows the client to publish messages to the topic `foo/${username}` with `retain` equal to `false` and a QoS of 0 or 1, for example, `foo/oximqtt_u`.
      "permission": "allow",
      "action": "publish",
      "topic": "foo/${username}",
      "retain": false,
