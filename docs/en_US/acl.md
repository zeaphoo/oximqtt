...
The built-in ACL has the lowest priority. If you want to disable it, you
can comment out all the rules. After the rules file is changed, OXIMQTT Broker needs to be restarted to make them take
effect.
</font>
</div>
...
## ACL Configuration in oximqtt.toml

The rules in the `[acl]` section of `oximqtt.toml` are matched from top to bottom in writing order.
...
The built-in ACL configuration in `oximqtt.toml` supports only the following placeholders in the subject's field (the 4th position of the
tuple).
...
::: tip Only a few simple and general rules are contained in the ACL configuration that make it a system-based ACL principle.
If you need to support complex, large amounts of ACL content, you should implement it in an external authentication module.

:::
<div style="width:100%;padding:15px;border-left:10px solid #1cc68b;background-color: #d1e3dd; color: #00b173;">
<div style="font-size:1.3em;">TIP<br></div>
<font style="color:#435364;font-size:1.1em;">
Only a few simple and general rules are contained in the ACL configuration that make it a system-based ACL principle.
If you need to support complex, large amounts of ACL content, you should implement it in an external authentication module.
</font>
</div>
