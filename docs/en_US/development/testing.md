...
# OXIMQTT Testing Guide

This document describes the OXIMQTT testing strategy, test layers, and how to run and extend the test suite.

---

## Test Layers

OXIMQTT has three testing layers:
...
The `oximqtt-test` crate provides a comprehensive test harness named `mqtt_harness`. It is a standalone binary that starts an OXIMQTT broker, runs test suites against it, and outputs structured reports.

### Building the Test Harness

```bash
# Build release binary (required — the harness runs oximqttd)
cargo build --release
...
OXIMQTT is tested against the [paho.mqtt.testing](https://github.com/eclipse/paho.mqtt.testing) suite.
...
```rust
use oximqtt_test::framework::testcase::{TestCase, TestResult};
use oximqtt_test::framework::context::TestContext;
...
Then register it in `oximqtt-test/src/tests/mod.rs` or the main entry point.
