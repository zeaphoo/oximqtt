...
server to store client authentication credentials or session information. *OXIMQTT* supports user authentication based on JWT.
...
Since the *OXIMQTT* JWT authenticator only verifies the JWT signature and cannot guarantee the legitimacy of the client's 
...
and issue a JWT for legitimate clients. The client will then use the issued JWT to connect to *OXIMQTT*.

#### Access Control List (Optional)

If the JWT contains an `acl` field, *OXIMQTT* will enforce access control for the client based on the permissions specified 
 ...
 * Algorithm(encrypt): Specifies the encryption method for the JWT. Optional values: `hmac-based` or `public-key`:
   * If `hmac-based` is selected, the JWT will use a symmetric key to generate and verify the signature (supporting 
     ...
     * Secret Base64 Encode(hmac_base64): Configure whether *OXIMQTT* needs to perform Base64 decoding on the Secret before 
...
By default, this built-in module is not enabled. To activate it, add the `[auth_jwt]` configuration section
in the main configuration file `oximqtt.toml`. The module is enabled when its configuration section is present.
