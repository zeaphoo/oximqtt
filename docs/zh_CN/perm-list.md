...
*OXIMQTT* 允许在认证阶段为客户端预设权限，用于控制客户端登录后的发布订阅权限检查。目前，JWT 认证和 HTTP 认证支持 *“权限预设”* ，使用权限列表 (ACL) 
...
```
如果所有ACL规则都没有被匹配，将被忽略并继续执行后续认证链，如果oximqtt-acl.toml中配置了["allow", "all"]规则，默认将通过所有被忽略的认证，
