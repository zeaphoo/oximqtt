...
内置 ACL 优先级最低，可以被 其它ACL 模块覆盖，如需禁用全部注释即可。规则文件更改后需重启 OXIMQTT服务 以应用生效。
</font>
</div>
...
内置的 `[acl]` 配置段在主题的域（元组的第四位）仅支持以下占位符：
...
<div style="font-size:1.3em;">提示<br></div>
<font style="color:#435364;font-size:1.1em;">
[acl] 配置段中应只包含一些简单而通用的规则，使其成为系统基础的 ACL 原则。如果需要支持复杂、大量的 ACL 内容，你应该在认证模块中去实现它。
</font>
</div>
