...
OXIMQTT MQTT Broker 的集中式配置管理系统。
...
pub struct Inner {
    ...
    pub acl: serde_json::Value,       // [acl] 节（内置 ACL 模块）
    pub retainer: serde_json::Value,  // [retainer] 节（内置保留消息模块）
    pub auth_jwt: serde_json::Value,  // [auth_jwt] 节（内置 JWT 认证模块）
    pub sys_topic: serde_json::Value, // [sys_topic] 节（内置系统主题模块）
...
| `cookie` | `String` | `"oximqttsecretcookie"` | 集群认证 cookie |
...
| `dir` | `String` | `"/var/log/oximqtt"` | 日志目录 |
...
4. `-f` / `--config` 指定路径（可选）
5. `OXIMQTT_*` 环境变量（如 `OXIMQTT_NODE_ID=1`, `OXIMQTT_RPC__SERVER_ADDR=0.0.0.0:5363`）
