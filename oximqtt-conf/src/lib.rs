...
//! Configuration management for the OXIMQTT broker.
//!
...
/// logging, listeners, plugins, and MQTT settings.
...
pub struct Inner {
    ...
    #[serde(default)]
    pub retainer: serde_json::Value,
    #[serde(default)]
    pub auth_jwt: serde_json::Value,
    #[serde(default)]
    pub sys_topic: serde_json::Value,
    #[serde(default)]
    pub mqtt: Mqtt,
    #[serde(default, skip)]
    pub opts: Options,
...
impl Settings {
    fn new(opts: Options) -> Result<Self> {
        let mut builder = Config::builder()
            .add_source(File::with_name("/etc/oximqtt/oximqtt").required(false))
            .add_source(File::with_name("/etc/oximqtt").required(false))
            .add_source(File::with_name("oximqtt").required(false))
            .add_source(
                config::Environment::with_prefix("oximqtt")
                    .try_parsing(true),
            );
    ...
    ///
    /// Outputs node ID, executor configuration, and overrides
...
impl Node {
    fn cookie_default() -> String {
        "oximqttsecretcookie".into()
    }
    // fn crash_dump_default() -> String {
    //     "/var/log/oximqtt/crash.dump".into()
    // }
}
